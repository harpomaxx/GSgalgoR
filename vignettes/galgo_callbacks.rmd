...
galgo(...,
  ...
  report_callback = default_callback,      # In the middle of the generation, 
                                           # right after the new mating pool have been created.
      ...) 
```


# A simple custom callback function definition
...
my_callback <-
    ...
    # code starts  here
    if (generation%%2 == 0)
        message(paste0("generation: ",generation," current_time: ",current_time))
    }
```
...
galgoR::galgo(generations = 6, 
              ...
              OS = OS,
  start_galgo_callback = galgoR::default_callback, 
  end_galgo_callback = galgoR::default_callback,   
  report_callback = my_callback,      # call `my_callback()` in the middle of of each generation/iteration.
  start_gen_callback = galgoR::default_callback,  
  end_gen_callback = galgoR::default_callback) 


```


# Saving the partial state of the population pool through a custom callback function
...
# Galgo implemented Callbacks

By default, Galgo implements four callback functions

`default_callback()` a simple callback that does nothing at all. It is just used for setting the default behavior of some of the hook-points inside `galgo()`
`base_report_callback()` a  report callback for printing basic information about the solution provided by `galgo()` such as fitness and crowding distance.
`no_report_callback()`   a report callback for informing the user galgo is running. Not valuable information is shown.
`base_return_pop_callback()` a callback function for building and returning the `galgo.Obj` object.
...
Notice by using the callback mechanism it is possible to modify even the returning value of the `galgo()` function. The default `base_return_pop_callback()` returns a `galgo.Obj` object, however it would simple to change that behavior for something like the `my_save_pop_callback()` and the function will not returning any value.
