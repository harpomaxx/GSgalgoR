...
properties of genetic algorithms in galgoR: A flexible yet robust
...
In the galgoR package, the NSGA-II framework was used for finding multiple
...
To start testing galgoR, we will use two Breast Cancer datasets. Namely, the
...
datasets. Additionally, we will use PAM50 centroids to perform breas cancer
sample classification. The datasets can be accesed from the following
...
- usegpu: logical default to FALSE, set to TRUE if you wish to use gpu computing
(gpuR package must be properly installed and loaded)
...
The current function restructurates a `galgo.Obj` to a more easy to understand
...
(LumA, LumB, Basal-like, HER2 and Normal-Like) have a strong overap with the
classical histopathological classification of BRCA tumors and migh affect
decission making when used to decided chemotherapy in certain cases.
...
strictly necessary to use galgoR (although feature selection might fasten GalgoR
runs), nevertheless, appropiate gene expression scaling is critical when runing
galgoR.
...
for(i in best_sol){
    ...
    CI_train<- 
        concordance.index(predict(Prediction.models[[i]],predicted_class_train_df),
    ...
    CI_test<- 
        concordance.index(predict(Prediction.models[[i]],predicted_class_test_df),
