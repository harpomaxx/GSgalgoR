...
knitr::include_graphics('./images/pareto1.jpg')
...
```{r echo=FALSE,eval=TRUE, out.width='100%'}
knitr::include_graphics('./images/pareto2.jpg')
...
RESULT<- non.dominated.summary(output=output,OS=train_surv, prob_matrix= train_expression, distancetype =distancetype, usegpu= usegpu )
...
```{r}
  CentroidsList <- create.centroids(output, solution.names = best_sol, train.set = train_expression)
```
...
### Test galgoR subtypes in GSE68465 (test) dataset

```{r}

train_classes<- classify.multiple(prob_matrix=train_expression,centroid.list= CentroidsList, distancetype = distancetype)


test_classes<- classify.multiple(prob_matrix=test_expression,centroid.list= CentroidsList, distancetype = distancetype)
...
for(i in best_sol){
  ...
  surv_formula <- as.formula(paste0("Surv~ ",i))
...
Compare Wilkerson classification vs Galgo classification in the GSE68465 dataset
```{r}
